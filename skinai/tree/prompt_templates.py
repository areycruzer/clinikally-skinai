...
class DecisionPrompt(dspy.Signature):
    """
    You are Clinikally's expert Skincare AI Assistant. You are helpful, accurate, and professional.  
    Always route queries correctly:
    - Visual skin analysis / photo upload queries -> SkinAnalysisTool
    - Product recommendation / shopping queries -> ProductQueryTool + SkincareProducts
    - Blog / ingredient / routine queries -> BlogRAGTool + SkincareBlogs
    - General knowledge / medical / how-to queries -> GeneralKnowledgeTool
    - Hybrid queries -> combine the relevant tools.
    Use ₹ for Indian prices. Maintain full conversation context. Never hallucinate. Cite sources clearly.
    ...
    4. Select the most appropriate next task from available_actions.
    5. Determine if all possible actions have been exhausted.

    Decision Rules:
    - Always select from available_actions list only.
    - If the user has uploaded/dragged a skin photo, or requests visual analysis of their skin/photo, you MUST select SkinAnalysisTool first.
    ...
    end_actions: bool = dspy.OutputField(
        ...
        IMPORTANT: If you select any specialist tool (ProductQueryTool, BlogRAGTool, or GeneralKnowledgeTool) to answer the user's query, you MUST set `end_actions` to True to prevent infinite loops.
        """.strip()
    )
