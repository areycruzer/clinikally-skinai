...
from skinai.objects import Result
from skinai.util.client import ClientManager
from skinai.util.parsing import format_dict_to_serialisable, remove_whitespace
...
class Environment:
    ...
    The environment is initialised with a default "SelfInfo.generic" key, which is a list of one object, containing information about SkinAI itself.
            ...
            self.environment["SelfInfo"]["info"] = [
                {
                    ...
                    "purpose": remove_whitespace(
                        """SkinAI is an agentic retrieval augmented generation (RAG) service, where users can query from Weaviate collections,
                    ...
                    SkinAI will dynamically display retrieved objects from the collections in the frontend.
                    SkinAI works via a tree-based approach, where the user's question is used to generate a tree of potential
                    ...
                    The backend of SkinAI is built with a range of libraries. SkinAI is built with a lot of base python, and was constructed from the ground up to keep it as modular as possible.
...
class CollectionData:
    ...
    async def set_collection_names(
        ...
        if len(self.removed_collections) > 0 and self.logger:
            self.logger.warning(
                "The following collections have not been pre-processed for SkinAI. "
...
class TreeData:
    ...
    def get_errors(self):
        if self.current_task == "skinai_decision_node":
            return self.errors
