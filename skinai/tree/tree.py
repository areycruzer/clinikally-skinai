...
# SkinAI
from skinai.config import Settings, SkinAIKeyManager
from skinai.config import settings as environment_settings
from skinai.tree.util import (
    DecisionNode,
    TreeReturner,
    Decision,
...
from skinai.tools.retrieval.aggregate import Aggregate
from skinai.tools.retrieval.query import Query
from skinai.tools.visualisation.visualise import Visualise
from skinai.tools.postprocessing.summarise_items import SummariseItems
from skinai.tools.text.text import (
    FakeTextResponse,
    CitedSummarizer,
)
from skinai.tree.util import ForcedTextResponse
from skinai.util.async_util import asyncio_run
from skinai.tree.objects import CollectionData, TreeData, Atlas, Environment
from skinai.util.client import ClientManager
from skinai.config import (
    check_base_lm_settings,
    check_complex_lm_settings,
    load_base_lm,
    load_complex_lm,
)
from skinai.util.objects import Tracker, TrainingUpdate, TreeUpdate
from skinai.util.parsing import remove_whitespace
from skinai.util.collection import retrieve_all_collection_names


class Tree:
    """
    The main class for the SkinAI decision tree.
    ...
    def __init__(
        ...
        use_skinai_collections: bool = True,
        ...
        Args:
            ...
            user_id (str): The id of the user, e.g. "123-456",
                unneeded outside of user management/hosting SkinAI app
            conversation_id (str): The id of the conversation, e.g. "123-456",
                unneeded outside of conversation management/hosting SkinAI app
            ...
            use_skinai_collections (bool): Whether to use weaviate collections as processed by SkinAI.
                If False, the tree will not use the processed collections.
            settings (Settings): The settings for the tree, an object of skinai.Settings.
        ...
        self.use_skinai_collections = use_skinai_collections
    ...
    def one_branch_init(self) -> None:
        ...
        from skinai.api.custom_tools import ProductQueryTool, BlogRAGTool, GeneralKnowledgeTool, SkinAnalysisTool
        self.add_tool(branch_id="base", tool=ProductQueryTool)
        self.add_tool(branch_id="base", tool=BlogRAGTool)
        self.add_tool(branch_id="base", tool=GeneralKnowledgeTool)
        self.add_tool(branch_id="base", tool=SkinAnalysisTool)
        ...
        if from_tool_ids == []:
            ...
            if new_branch_id not in self.decision_nodes:
                decision_node = DecisionNode(
                    ...
                    root=False,
                    logger=self.settings.logger,
                    use_skinai_collections=self.use_skinai_collections,
                )
    ...
    async def create_conversation_title_async(self) -> str:
        ...
        """
        with SkinAIKeyManager(self.settings):
    ...
    async def async_run(
        ...
        **kwargs,
        ...
        """
        Async version of .run() for running SkinAI in an async environment.
        ...
        if _first_run:
            ...
            if client_manager.is_client:
                ...
                if self.use_skinai_collections:
                    try:
                        if collection_names == []:
                        ...
                        self.settings.logger.error(f"Failed to initialize Weaviate collections (graceful degradation): {e}")
        ...
        while True:
            ...
            # Under normal circumstances decide from the decision node
                ...
                self.tree_data.set_current_task("skinai_decision_node")
                with SkinAIKeyManager(self.settings):
            ...
            if action_fn is not None:
                ...
                with SkinAIKeyManager(self.settings):
                    try:
                        ...
                        self.settings.logger.error(f"Error during tool {self.current_decision.function_name} execution: {e}")
                        successful_action = False

                if not successful_action:
                    if self.current_decision.function_name != "GeneralKnowledgeTool":
                        self.settings.logger.info("Falling back to GeneralKnowledgeTool due to tool failure...")
                        from skinai.objects import Status
                        fallback_msg = "I'm having trouble accessing my database right now, so I will answer you based on my general clinical skincare knowledge! 🧴✨"
                        action_result, _ = await self._evaluate_result(
                            Status(fallback_msg),
                            self.current_decision
                        )
                        ...
                        fallback_tool = self.tools["GeneralKnowledgeTool"]
                        fallback_inputs = {"search_query": self.user_prompt}
                        try:
                            async for result in fallback_tool(
                                tree_data=self.tree_data,
                            ...
                            self.settings.logger.error(f"Fallback GeneralKnowledgeTool also failed: {ex}")
                            completed = True
                    else:
                        completed = (
                            False
    ...
    def run(
        ...
        """
        Run the SkinAI decision tree.

        Args:
            user_prompt (str): The input from the user.
            collection_names (list[str]): The names of the collections to use.
                If not provided, SkinAI will attempt to retrieve all collection names from the client.
            ...
            query_id (str): The id of the query.
                Only necessary if you are hosting SkinAI on a server with multiple users.
    ...
    def import_from_json(cls, json_data: dict) -> "Tree":
        ...
        tree = cls(
            ...
            use_skinai_collections=json_data["use_skinai_collections"],
            settings=settings,
        )
