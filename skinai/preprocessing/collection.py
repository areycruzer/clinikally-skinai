...
from skinai.config import Settings, load_base_lm, SkinAIKeyManager
from skinai.config import settings as environment_settings

# Fast, lightweight dummy nlp function for estimating token counts via whitespace splitting
def nlp(text):
    class DummyDoc:
        def __init__(self, t):
...
from skinai.util import return_types as rt

from skinai.preprocessing.prompt_templates import (
    CollectionSummariserPrompt,
    DataMappingPrompt,
    ReturnTypePrompt,
    PromptSuggestorPrompt,
)
from skinai.util.collection import async_get_collection_data_types
from skinai.util.async_util import asyncio_run
from skinai.util.parsing import format_dict_to_serialisable
...
async def preprocess_async(
    ...
    from the fields to the frontend-specific fields in SkinAI.
...
def preprocess(
    ...
    This function saves the output into a collection called ELYSIA_METADATA__, which is automatically called by SkinAI.
...
def edit_preprocessed_collection(
    ...
    Find available mappings in the `skinai.util.return_types` module.
