...
from skinai.api.services.tree import TreeManager
from skinai.objects import Update
from skinai.util.client import ClientManager
from skinai.api.core.log import logger
from skinai.api.utils.config import Config
from skinai.api.utils.config import FrontendConfig
from skinai.tree.util import get_saved_trees_weaviate
...
async def load_frontend_config_from_file(
    ...
    try:
        skinai_package_dir = Path(__file__).parent.parent.parent  # Gets to skinai/
        config_dir = skinai_package_dir / "api" / "user_configs"
...
class UserManager:
    ...
    The user manager/tree manager is decoupled from the core of the SkinAI package functionality.
    It is designed to be used to manage separate SkinAI instances, set of trees (via tree managers), configs, etc.
    """
    ...
    async def process_tree(
        ...
        if self.check_tree_timeout(user_id, conversation_id):
            try:
                import asyncio
                # Add a 5-second timeout to prevent hanging when Weaviate is offline/unresponsive
                exists = await asyncio.wait_for(
                    self.check_tree_exists_weaviate(user_id, conversation_id),
                    timeout=5.0
                )
                if exists:
                    await asyncio.wait_for(
                        self.load_tree(user_id, conversation_id),
                        timeout=5.0
                    )
                else:
                    logger.info("Tree does not exist in Weaviate, creating local tree.")
                    await self.initialise_tree(user_id, conversation_id)
            except Exception as e:
                logger.warning(f"Weaviate offline or tree loading failed: {e}. Falling back to a clean local tree.")
                try:
                    await self.initialise_tree(user_id, conversation_id)
                except Exception as e2:
                    logger.error(f"Failed to initialise fallback local tree: {e2}")
