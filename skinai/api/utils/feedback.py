...
async def create_feedback(
    ...
    # Sanitize tasks_completed to match Weaviate's schema exactly
    sanitized_tasks_completed = []
    for task_prompt in history["tree_data"].tasks_completed:
        prompt_val = task_prompt.get("prompt", "")
        # The key in tree_data can be "task" or "tasks"
        raw_tasks = task_prompt.get("task", task_prompt.get("tasks", []))
        ...
        for t in raw_tasks:
            ...
            task_name = str(t.get("task", ""))
            reasoning = str(t.get("reasoning", ""))
            todo = str(t.get("todo", ""))
            action = bool(t.get("action", False))
            
            count_val = t.get("count", 0)
            try:
                count = float(count_val) if count_val is not None else 0.0
            except (ValueError, TypeError):
                count = 0.0
                
            # Collect all other fields into extra_string
            extra_data = {}
            for k, v in t.items():
                if k not in ["task", "reasoning", "todo", "action", "count"]:
                    extra_data[k] = v
                    
            extra_string = ""
            if extra_data:
                try:
                    extra_string = json.dumps(extra_data)
                except Exception:
                    extra_string = str(extra_data)
        ...
        sanitized_tasks_completed.append({
            "prompt": prompt_val,
    ...
    properties = {
        ...
        "tasks_completed": sanitized_tasks_completed,
