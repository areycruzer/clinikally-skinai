...
class Settings:
    """
    Settings for SkinAI.
    This class handles the configuration of various settings within SkinAI.
    This includes:
    ...
    The Settings object is set as at a default `settings` object if running SkinAI as a package.
    You can import this via `skinai.config.settings`.
    ...
    The Settings object can be passed to different SkinAI classes and functions, such as `Tree` and `preprocess`.
    These will not use the global `settings` object, but instead use the Settings object you passed to them.
    """

    def __init__(self):
        """
        Initialize the settings for SkinAI.
    ...
    def configure(
        ...
        **kwargs,
        ...
        """
        Configure the settings for SkinAI for the current Settings object.
        ...
        if "base_model" in kwargs:
            if "base_provider" not in kwargs:
                raise ValueError(
                    ...
                    "E.g. `skinai.config.configure(base_model='gpt-4o-mini', base_provider='openai')`"
                )
            ...
            if kwargs["base_provider"] == "ollama" and (
                ...
                raise ValueError(
                    ...
                    "e.g. `skinai.config.configure(model_api_base='http://localhost:11434')`"
                )
        ...
        if "complex_model" in kwargs:
            if "complex_provider" not in kwargs:
                raise ValueError(
                    ...
                    "E.g. `skinai.config.configure(complex_model='gpt-4o', complex_provider='openai')`"
                )
...
class SkinAIKeyManager:
    ...
    def _check_model_availability(self, model: str, provider: str) -> bool:
        if provider not in models_by_provider and not provider.startswith("openrouter"):
            ...
            provider in models_by_provider
            and provider != "openrouter"
            and model not in models_by_provider[provider]
        ):
            raise IncorrectModelError(
                f"The model {model} is not available for provider {provider}. "
                f"Some example models for {provider} are: "
...
# but used when using SkinAI as a package
settings = Settings()
settings.smart_setup()
...
def configure(**kwargs) -> None:
    """
    Configure the settings for SkinAI for the global settings object.
